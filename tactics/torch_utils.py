...
def is_parallel(model):
    ...
    return type(model) in (nn.parallel.DataParallel, nn.parallel.DistributedDataParallel, BalancedDataParallel)
    ...
    for k, v in yolo5_sd.items():
        ...
        for bk in bkbo_keys:
            if bk in k:
                ...
                break
        if bkbo_k:
            new_k = k.replace('model.', 'backbone.backbone.')
        else:
            idx = k.split('.')[1]
            new_idx = str(int(idx) - 10)
            new_k = '.'.join(['head.head', new_idx] + k.split('.')[2:])
        convert_sd[new_k] = v
    del yolo5_sd
    return convert_sd

def save_load_state_dict(model, ckpt_state_dict):
    ckpt_keys = list(ckpt_state_dict.keys())
    if 'backbone.backbone' not in ckpt_keys[0]:
        convert_state_dict = state_dict_convert_yolo5_2_flexible(ckpt_state_dict)
    else:
        convert_state_dict = ckpt_state_dict
        
    update_state_dict = {}
    for key, val in model.state_dict().items():
        if key in convert_state_dict.keys() and convert_state_dict[key].shape == val.shape:
            update_state_dict[key] = val
        else:
            print(f"Mismatch key: {key}")
    model.load_state_dict(update_state_dict, strict=False)  # load

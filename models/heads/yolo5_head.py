...
class YoloV5Head(nn.Module):
    def __init__(self, variant='s', nc=80, in_fpn_feats={}, anchors=ANCHORS):  # model, input channels, number of classes
        ...
        '''
        gh, gw = YOLO5_VARIANT[variant]['depth_multiple'], YOLO5_VARIANT[variant]['width_multiple']
        ...
        detect_layer = Detect(nc=nc, anchors=anchors, ch=fpn_chs)
    ...
    def _set_layers_attr(self, profile=False):
        head_params = [
                        [-1,                 1, 'Conv', [self.p2_ch, 1, 1]],                            # idx-0  
                        [-1,                 1, 'nn.Upsample', ['None', 2, 'nearest']],                 # idx-1  
                        [[-1, 'p2'],         1, 'Concat', [1]],                                         # idx-2  
                        [-1,                 3, 'C3', [self.p2_ch, False]],                             # idx-3  
                        [-1,                 1, 'Conv', [self.p1_ch, 1, 1]],                            # idx-4  
                        [-1,                 1, 'nn.Upsample', ['None', 2, 'nearest']],                 # idx-5  
                        [[-1, 'p1'],         1, 'Concat', [1]],                                         # idx-6  
                        [-1,                 3, 'C3', [self.p1_ch, False]],                             # idx-7  
                        [-1,                 1, 'Conv', [self.p1_ch, 3, 2]],                            # idx-8  
                        [[-1, 'h1'],         1, 'Concat', [1]],                                         # idx-9  
                        [-1,                 3, 'C3', [self.p2_ch, False]],                             # idx-10 
                        [-1,                 1, 'Conv', [self.p2_ch, 3, 2]],                            # idx-11 
                        [[-1, 'h0'],         1, 'Concat', [1]],                                         # idx-12 
                        [-1,                 3, 'C3', [self.p3_ch, False]],                             # idx-13 
                        [['d1', 'd2', 'd3'], 1, 'Detect', ['nc', 'anchors']]]
...
if __name__ == '__main__':
    head_input_s = torch.rand((8, 512, 20, 20))
    pyramid_feats = [torch.rand(8,128,80,80), torch.rand(8,256,40,40), torch.rand(8,512,20,20)]
    head = YoloV5Head(variant='s', nc=80, in_fpn_feats=pyramid_feats)
    out = head(pyramid_feats)
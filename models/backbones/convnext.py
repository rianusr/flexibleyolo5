...
class ConvNeXtBackbone(nn.Module):
    ...
    """
    def __init__(self, variant='s', in_chans=3, focus_ch=32, drop_path_rate=0., layer_scale_init_value=1e-6):
        super().__init__()

        depths = PARAMS[variant]['depths']
        dims   = PARAMS[variant]['dims']
        
        self.focus = Conv(in_chans, focus_ch, 6, 2, 2) if focus_ch > 3 else nn.Identity()
